...
public class GazelleParser : IParseIndexerResponse
{
        ...
        foreach (var result in jsonResponse.Resource.Response.Results)
        {
            ...
            if (result.Torrents != null)
            {
                foreach (var torrent in result.Torrents)
                {
                    ...
                    if (Settings.UseFreeleechToken == (int)GazelleUseFreeleechTokens.Required && !torrent.CanUseToken)
                    {
                        continue;
                    }
            ...
            {
                // skip releases that cannot be used with freeleech tokens when the option is enabled
                if (Settings.UseFreeleechToken == (int)GazelleUseFreeleechTokens.Required && !result.CanUseToken)
                {
                    continue;
                }
    ...
    protected virtual string GetDownloadUrl(int torrentId, bool canUseToken)
    {
        ...
        if (Settings.UseFreeleechToken == (int)GazelleUseFreeleechTokens.Required && canUseToken)
        {
